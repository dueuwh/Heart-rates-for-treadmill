...
def load_data(base_path, folder):
    ...
    for i in range(len(y_total)):
        ...
        index = np.load(f"{base_path}{folder}/{index_total[i]}")
...
def adjust_R(sensor1_measure, sensor2_measure, base_R1=1.0, base_R2=4.0, factor=0.5, min_R1=0.5, min_R2=2.0, threshold=3.0):
    """
    ...
    if abs(sensor2_measure - sensor1_measure) < threshold:
        adjusted_R1 = base_R1
        adjusted_R2 = max(base_R2 * factor, min_R2)
    else:
        # 값이 같을 경우 기본값 유지
...
def run_ukf(t, true_state, sensor1, sensor2):
    ...
    ukf.P = np.array([[1.0]])  # 초기 오차 공분산
    ukf.R = np.array([[10.0, 0.0],
...
def plot_results(t, true_state, sensor1, sensor2, ukf_estimates, title, index, save_path, algorithm):
    ...
    print("index: ", index)
    plt.axvline(0, 3, 200, color='red', linestyle='--', linewidth=2)
    plt.text(30, 70, 0, ha='center', va='bottom', fontsize=15, color='red')
    for value in index:
        plt.axvline(value[1], 0, 200, color='red', linestyle='--', linewidth=2)
        plt.text(value[1]+30, 70, value[0], ha='center', va='bottom', fontsize=20, color='red')
    plt.legend()
    # plt.grid()
...
def main():
    ...
    for folder in folders:
        ...
        for t, true_state, sensor1, sensor2, title, index in data_loader:
            ...
            save_path = f"{save_base}{folder}/{title}_total_result.png"
            plot_results(t, true_state, sensor1, sensor2, ukf_estimates, title, index.tolist(), save_path, folder)
